layui.use(['element', 'carousel', 'layer', 'laypage', 'util', 'form'], function () {
    ...
    //取浏览器地址的参数
    let topicId = window.location.search.split("=")[1]

    let isFirst = true

    let getTopic = function (page) {
        $.ajax({
            url: '../topic',
            type: "post",
            data: {
                method: "findDetailByTopicId",
                topicId,
                page
            },
            async: false,
            success(res) {
                if (isFirst) {
                    /** 渲染主题详情 */
                    let topicItem = $(`
                        <div class="topicItem" topicId=${res[0].id}>
                            <div><h1>${res[0].title}</h1></div>
                            <hr >
                            <div>${(res[0].content)}</div>
                            <hr >
                            <div style="display: flex;justify-content: flex-end;align-items: baseline">浏览量：<span style="color: red">${res[0].pv}</span></div>
                            <hr>
                            <div  style="display: flex;justify-content: flex-end;align-items: baseline">
                            <button type="button" class="layui-btn layui-btn-radius comBtn">评论</button></div>
                        </div>
`)
                ...
                laypage.render({
                    ...
                    count: res[1].totalRecord, //数据总数，从服务端得到
                    limit: res[1].pageSize, //每页的最大展示条数
            ...
            <div class="cItemLeft">
                <img src="" class="layui-nav-img commentUserHeadImg" style="height: 100%;width: auto;">
            </div>
            <div class="cItemRight">
                <p style="font-size: 18px;margin-top: 0"><span class="commentUserName"></span> <span class="commentTime">${res[1].list[i].createTime.date.year + "-" + res[1].list[i].createTime.date.month + "-" + res[1].list[i].createTime.date.day}</span>
                </p>
                <div class="commentText">
                    ${res[1].list[i].content}
                </div>
                <div style="display: flex;justify-content: flex-end;align-items: baseline">
                    <button type="button" class="layui-btn layui-btn-radius comBtn">回复</button>
                </div>
            </div>
        </div>
                    `)
                    $(".commentWrap").append(commentItem)
                    setCommentUser(getUser(res[1].list[i].userId))
                }
            },
            error(err) {
                console.log(err)
    ...
    let setCommentUser = function (user) {
        $(".commentUserName").text(user.userName)
        $(".commentUserHeadImg").attr("src", user.img)
    }

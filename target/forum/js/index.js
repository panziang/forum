...
layui.use(['element', 'carousel', 'layer', 'laypage', 'util', 'form'], function () {
    ...
    //轮播图建造实例  ins.reload()  重置
    let ins = carousel.render({
        elem: '#slideshow',
        width: '70%', //设置容器宽度,
        arrow: 'anim' //始终显示箭头
        //always,anim: 'updown' //切换动画方式
    });
    ...
    /** 轮播图的跳转 */
    $(".slideshowWrap").on("click", ".slideshowDetail", function () {
        window.open(`./category.html?c_id=${$(this).attr("c_id")}`)
        // window.location.href = `./category.html?c_id=${$(this).attr("c_id")}`
    })
    ...
    $("#searchBtn").on("click", function () {
        ...
        if (searchKey == "") {
            searchKey = $("#searchInput").attr("placeholder")
        }
        if (userId == null) {
            layer.msg('请先登录', {
                    icon: 4,
                ...
                for (let i = 0; i < rows; i++) {
                    let topicItem = $(`
                        <div class="topicItem" topicId=${res.list[i].id}>
                            <div><h1>${res.list[i].title}</h1></div>
                            <hr >
                            <div>${(res.list[i].content).substring(0, 200) + '">'}...</div>
                            <hr >
                            <div style="display: flex;justify-content: flex-end;align-items: baseline">浏览量：<span style="color: red">${res.list[i].pv}</span></div>
                        </div>
`)
